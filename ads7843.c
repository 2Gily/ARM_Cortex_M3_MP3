...
extern volatile unsigned short tick, jiffes,volume;
...
void touch_adjust(unsigned sw)
{
	...
	sdata.data[0] = eeprom_read(0);
	sdata.data[1] = eeprom_read(1);
	if (sdata.reg == 0x0517)
	{
		sdata.data[0] = eeprom_read(2);
		sdata.data[1] = eeprom_read(3);
		pen_point.xfac = sdata.reg;
		sdata.data[0] = eeprom_read(4);
		sdata.data[1] = eeprom_read(5);
		pen_point.xoff = sdata.reg;
		sdata.data[0] = eeprom_read(6);
		sdata.data[1] = eeprom_read(7);
		pen_point.yfac = sdata.reg;
		sdata.data[0] = eeprom_read(8);
		sdata.data[1] = eeprom_read(9);
		pen_point.yoff = sdata.reg;
		//lcd_printf(5,1,"toutch start");
		if (sw == 0)
			return;
	}
	...
	while (1)
	{
		...
		if (pen_status == KEY_DOWN)
		{
			...
			case 4:
				...
				sdata.reg = pen_point.xfac;
				eeprom_write(2, sdata.data[0]);
				eeprom_write(3, sdata.data[1]);
				sdata.reg = pen_point.xoff;
				eeprom_write(4, sdata.data[0]);
				eeprom_write(5, sdata.data[1]);
				sdata.reg = pen_point.yfac;
				eeprom_write(6, sdata.data[0]);
				eeprom_write(7, sdata.data[1]);
				sdata.reg = pen_point.yoff;
				eeprom_write(8, sdata.data[0]);
				eeprom_write(9, sdata.data[1]);
				sdata.reg = 0x0517;
				eeprom_write(0, sdata.data[0]);
				eeprom_write(1, sdata.data[1]);
				wait_ms(500);
...
// {
// 	lcd_printf(5, 10, "touch_process code execute!!!");
// 	if (pen_status == KEY_DOWN)
// 	{
...
// 		{
// 			pen_point.x0 = (pen_point.xfac * pen_point.x) / 10000 + pen_point.xoff;
// 			pen_point.y0 = 319 - ((pen_point.yfac * pen_point.y) / 10000 + pen_point.yoff);
// 			lcd_printf(5, 3, "X:%5d Y%5d ", pen_point.x0, pen_point.y0);

// 			//���� ����  0~50 x ǥ�� --> x>0 && x<50  , // 240
// 			if ((pen_point.x0 < 60) && (pen_point.y0 >= 120 && pen_point.y0 < 315))
// 			{
// 				play_next();
// 			}
// 			if ((pen_point.x0 >= 60 && pen_point.x0 < 120) && (pen_point.y0 >= 280 && pen_point.y0 < 315))
// 			{
// 				play_mp3 = 0;
// 			}
// 			if ((pen_point.x0 >= 120 && pen_point.x0 < 180) && (pen_point.y0 >= 280 && pen_point.y0 < 315))
// 			{
// 				play_mp3 = 1;
// 			}
// 			if ((pen_point.x0 >= 180 && pen_point.x0 < 240) && (pen_point.y0 >= 280 && pen_point.y0 < 315))
// 			{
// 				play_prev();
// 			}
// 		}
...
void touch_process(void)
{
	if (pen_status == KEY_DOWN)
	{
		...
		if (read_ads7843_2(&pen_point.x, &pen_point.y))
		{
			pen_point.x0 = (pen_point.xfac * pen_point.x) / 10000 + pen_point.xoff;
			pen_point.y0 = 319 - ((pen_point.yfac * pen_point.y) / 10000 + pen_point.yoff);
			lcd_printf(2, 0,"X:%5d Y:%5d", pen_point.x0, pen_point.y0);

			if ((pen_point.x0 >= 0 && pen_point.x0 < 80) && (pen_point.y0 >= 245 && pen_point.y0 < 305))
			{
				play_prev();
			}
			if ((pen_point.x0 >= 80 && pen_point.x0 < 160) && (pen_point.y0 >= 245 && pen_point.y0 < 305))
			{
				if (play_mp3 == 0)
				{
					play_mp3 = 1;
			...
			if ((pen_point.x0 >= 160 && pen_point.x0 < 240) && (pen_point.y0 >= 245 && pen_point.y0 < 305))
			{
				play_next();
			}
			if ((pen_point.x0 >= 200 && pen_point.x0 < 230) && (pen_point.y0 >= 10 && pen_point.y0 < 40))
			{
			...
			if ((pen_point.x0 >= 200 && pen_point.x0 < 230) && (pen_point.y0 >= 50 && pen_point.y0 < 80))
			{
				if (volume > 0 && volume <= 128)
				{
					if(volume+5 > 127)
