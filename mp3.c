...
void display_id3v1_tag (void)
{
      ...
      if ((buff[0] == 'T') && (buff[1] == 'A') && (buff[2] == 'G')) {
         ...
         lcd_printf(1,8,"Tttle: %s ",ID3V1_Buf.Title);
         lcd_printf(1,10,"Artist: %s ",ID3V1_Buf.Artist);
         lcd_printf(1,12,"Album: %s ",ID3V1_Buf.Album);
      }
      f_lseek(&fsrc,0);
}
...
void mp3_seek (unsigned char sw)
{
       if (sw) {
       	  seek_value += 8192;
       	  if (seek_value >= (int)fsrc.fsize) seek_value = fsrc.fsize;
       } else {
       	  seek_value -= 8192;
...
{
   mp3_flag = 0;
   if ((datasize >= fsrc.fsize) || (fsrc.fsize == 0))
      mp3_flag = 1;
